            ...
            switch (strtolower($args[0])) {
                ...
                case 'delete':
                    if ($sender->hasPermission('mineshop.command.delete') || $sender->hasPermission(DefaultPermissions::ROOT_OPERATOR)) {
                        (new MineMenu($this->owningPlugin))->open($sender, true);
                        return;
                    }
                    break;
                default:
                    if ($sender->hasPermission('mineshop.command.add') || $sender->hasPermission('mineshop.command.delete') || $sender->hasPermission(DefaultPermissions::ROOT_OPERATOR)) {
